...
async function processVideo(video, progress) {
  console.log(`\n${'═'.repeat(70)}`);
  console.log(`🎬 #${video.index}: ${video.title.substring(0, 60)}${video.title.length > 60 ? '...' : ''}`);
...
async function main() {
  ...
  for (let i = 0; i < videosToProcess.length; i += batchSize) {
    ...
    console.log(`\n${'━'.repeat(70)}`);
    console.log(`📦 Batch ${batchNum}: ${i + 1}-${Math.min(i + batchSize, videosToProcess.length)}`);
    console.log(`${'━'.repeat(70)}`);
  ...
  console.log(`📊 Total: ${progress.completed.length}/${allVideos.length} (${((progress.completed.length/allVideos.length)*100).toFixed(1)}%)\n`);
}
