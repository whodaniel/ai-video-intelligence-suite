...
function findInsertButton() {
  ...
  let btn = document.querySelector('[data-test-id="add-media-button"]') || 
            document.querySelector('[data-test="selectMediaMenu"]');
  if (btn) return btn;

  // Strategy 2: Robust search
  const buttons = Array.from(document.querySelectorAll('button, [role="button"], .mat-mdc-icon-button, .mat-icon-button'));
  return buttons.find(b => {
    ...
    const hasSvgPlus = innerHtml.includes('<path') && (innerHtml.includes('d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"') || innerHtml.includes('d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11h-4v4h-2v-4H7v-2h4V7h2v4h5v2z"'));
...
async function dismissOverlays() {
  ...
  for (const btn of dismissButtons) {
    if (btn.offsetParent !== null) { // Only if visible
...
async function checkForErrors() {
  ...
  if (document.body.innerText.includes('Sign in to') || document.body.innerText.includes('Choose a project')) {
    sendLog('Detected sign-in or project selection page - automation blocked', 'error');
    throw new Error('AUTH_OR_PROJECT_REQUIRED');
  }

  const errorElements = document.querySelectorAll('[role="alert"], .error-message, .mat-error, .mat-snack-bar-container');
...
async function ensureCorrectModel() {
  ...
  const modelBtn = modelButtons.find(b =>
  ...
  if (modelBtn) {
    ...
    const menuItems = document.querySelectorAll('[role="menuitem"], [role="option"], .mat-menu-item');
...
async function addYouTubeVideo(url, startTime = 0, endTime = null) {
  ...
  for (let attempt = 0; attempt < 15; attempt++) {
    ...
    await dismissOverlays();
    await sleep(2000);
    if (attempt % 3 === 0) sendLog(`Waiting for Insert button (attempt ${attempt + 1}/15)...`, 'info');
  }
  ...
  sendLog(`Found Insert button: ${insertBtn.getAttribute('aria-label') || 'icon-search'}`, 'success');
  await clickElement(insertBtn, 'Insert button');
  await sleep(1500);

  // Search for the YouTube Video option in the menu (Robust search)
  let ytBtn = document.querySelector('.youtube-video-menu-item') || 
              document.querySelector('[data-test-id="youtube-video-menu-item"]');
  
  if (!ytBtn) {
    const menuItems = Array.from(document.querySelectorAll('button, [role="menuitem"], .mat-mdc-menu-item'));
  ...
  if (!ytBtn) {
    ...
    const allMenu = Array.from(document.querySelectorAll('[role="menuitem"], .mat-mdc-menu-item'));
    sendLog(`Available menu items: ${JSON.stringify(allMenu.map(el => el.textContent.trim()))}`, 'debug');
  ...
  if (inputs.length > 1) {
    const foundInput = Array.from(inputs).find(input => {
      ...
      return ph.includes('url') || ph.includes('link') || ph.includes('youtube') || type === 'url';
    });
  ...
  const saveBtn = dialogButtons.find(b => 
...
async function ensurePaidApiKey() {
  ...
  const noKeyBtn = document.querySelector('.paid-api-key-card[aria-label="No API Key"]') || 
                   Array.from(document.querySelectorAll('.paid-api-key-card')).find(el => el.textContent.includes('No API Key'));
  ...
  if (noKeyBtn) {
    ...
    // 1. Select Project
    const projectSelect = document.querySelector('mat-select[aria-label="Select a paid project"]');
    if (projectSelect) {
    ...
    const saveToggle = document.querySelector('button[role="switch"][aria-labelledby="save-paid-api-key-label"]') || 
    ...
    // 3. Confirm
    const confirmBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.trim() === 'Select key');
    if (confirmBtn) {
...
async function runAnalysis() {
  ...
  if (!runBtn) {
    ...
    runBtn = buttons.find(b => {
      ...
      const isSideMenu = aria.includes('settings') || text.includes('settings') || aria.includes('history');
      ...
      return (text.includes('run') || aria.includes('run')) && 
...
async function processTask(task) {
  ...
  try {
    switch (type) {
      case 'GET_DURATION':
        const duration = await withRetry(
