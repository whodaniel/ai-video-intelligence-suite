...
const isCustomApp = () => {
  return window.location.hostname.endsWith('.usercontent.goog') ||
...
const extractQueueFromDOM = () => {
  ...
  const tableRows = document.querySelectorAll('table tbody tr, [class*="queue"] tr, [class*="video"] tr');
  ...
  if (queue.length === 0) {
    ...
    const allMatches = allText.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/g) || [];
...
}, true); // Use capture phase to intercept before the app
...
console.log('[AI Video Suite] Iframe Bridge: Signaling extension ready to', window.location.hostname);
